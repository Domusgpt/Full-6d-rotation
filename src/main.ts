import { HypercubeCore, type RotationComputationMode } from './core/hypercubeCore';
...
let rotationMode = (rotationModeSelect.value as RotationComputationMode) ?? 'sequential';
const rotationModeLabels: Record<RotationComputationMode, string> = {
  sequential: 'Sequential Planes',
  matrix: 'Matrix Multiplication',
  dualQuaternion: 'Dual Quaternion'
};
let rotationComputation: RotationComputationMode = rotationMode;
...
rotationModeSelect.addEventListener('change', (event) => {
  rotationMode = (event.target as HTMLSelectElement).value as RotationComputationMode;
  rotationComputation = rotationMode;
  core.setRotationMode(rotationMode);
  if (latestDynamics) {
    updateStatus(latestDynamics);
  }
});
...
function updateStatus(dynamics: RotationDynamics) {
  const rotationLabel = rotationModeLabels[rotationComputation];
  statusEl.textContent =
    `${statusBase} · solver ${rotationLabel} · source ${rotationSource} · energy ${(dynamics.energy * 100).toFixed(0)}% · chaos ${(dynamics.chaos * 100).toFixed(0)}%`;
}
