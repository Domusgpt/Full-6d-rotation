import { HypercubeCore, RotationMode } from './core/hypercubeCore';
...
const rotationModeSelect = document.getElementById('rotationMode') as HTMLSelectElement;
...
const core = new HypercubeCore(canvas, {
  ...
  rotationMode: rotationModeSelect.value === 'dualQuaternion' ? RotationMode.DualQuaternion : RotationMode.Sequential
});

if (rotationModeSelect.value === 'dualQuaternion') {
  statusRotation = 'Rotation Path: Dual Quaternion Spinor';
}
...
let statusRotation = 'Rotation Path: Sequential Six-Plane';
...
function setGeometry(id: GeometryId) {
  ...
  statusGeometry = `Geometry: ${id} · vertices ${vertexCount} · edges ${edgeCount}`;
}
...
rotationModeSelect.addEventListener('change', (event) => {
  const select = event.target as HTMLSelectElement;
  const mode = select.value === 'dualQuaternion' ? RotationMode.DualQuaternion : RotationMode.Sequential;
  core.setRotationMode(mode);
  statusRotation =
    mode === RotationMode.DualQuaternion
      ? 'Rotation Path: Dual Quaternion Spinor'
      : 'Rotation Path: Sequential Six-Plane';
  pushRotationSnapshot(performance.now());
});
...
function updateStatus(dynamics: RotationDynamics) {
  statusEl.textContent =
    `${statusGeometry} · ${statusRotation} · energy ${(dynamics.energy * 100).toFixed(0)}% · chaos ${(dynamics.chaos * 100).toFixed(0)}%`;
}
