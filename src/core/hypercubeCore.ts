...
void main() {
  ...
  float normSpatialSpeed = clamp(spatialSpeed / (PI * 4.0), 0.0, 1.0);
  float normHyperSpeed = clamp(hyperSpeed / (PI * 4.0), 0.0, 1.0);
  float velocityFlux = clamp((normSpatialSpeed + normHyperSpeed) * 0.5, 0.0, 1.0);
  float velocityWeave = clamp(normSpatialSpeed - normHyperSpeed, -1.0, 1.0);
  ...
  hueShift += (spatialPulse - hyperPulse) * 0.1 + velocityFlux * 0.18 + velocityWeave * 0.12;
  float isoSaturation = clamp(saturation + (isoPhase - 0.5) * 0.22 + confidence * 0.1 + velocityFlux * 0.12, 0.0, 1.0);
  float isoBrightness = clamp(
    brightness + (1.0 - chiralTone) * 0.12 + planeEnergy * 0.08 + spatialPulse * 0.15 + velocityFlux * 0.18,
    0.0,
    1.2
  );
  float isoThickness = thickness * (1.0 + chiralTone * 0.45 + planeEnergy * 0.25);
  isoThickness *= mix(0.6, 1.6, confidence);
  isoThickness *= mix(0.85, 1.55, velocityFlux);
  float isoChaosBase = clamp(
    chaos + chiralTone * 0.35 + planeBalance * 0.2 + hyperPulse * 0.1 + velocityFlux * 0.25 + velocityWeave * 0.12,
    0.0,
    1.0
  );
  float isoEnergyBase = clamp(mix(energy, 1.0, chiralTone * 0.15) + planeEnergy * 0.1 + velocityFlux * 0.2, 0.0, 1.2);
  ...
  isoThickness *= (1.0 + orientationWeave * 0.12 + velocityWeave * 0.08);
  float isoChaos = clamp(isoChaosBase + sixPlaneWeight * 0.15 + velocityFlux * 0.2, 0.0, 1.0);
  float isoEnergy = clamp(isoEnergyBase + sixPlaneWeight * 0.12 + velocityFlux * 0.18, 0.0, 1.3);
  ...
  float shimmer = 0.6 + 0.4 * sin(u_time * 0.6 + depth * 0.4 + spatialPulse * 2.2 + timeSeconds * 0.5 + velocityFlux * 1.1 + velocityWeave * 0.8);
  vec3 harmonicColor = mix(neutral, baseColor, isoSaturation) * contour * shimmer;
  float weaveHue = fract(harmonic + planeBalance * 0.18 + sixPlaneWeight * 0.22 + velocityWeave * 0.2);
  vec3 planeColor = spectralPalette(weaveHue);
  float planeMix = clamp(0.18 + sixPlaneWeight * 0.5 + abs(planeBalance) * 0.22 + velocityFlux * 0.25, 0.0, 0.95);
