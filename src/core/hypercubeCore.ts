...
export class HypercubeCore {
  ...
  private rotationMode: RotationMode = RotationMode.Sequential;
  ...
  private render(time: number, deltaTime: number) {
    ...
    const modeIndex = this.rotationMode === RotationMode.Sequential ? 0 : 1;
    gl.uniform1i(this.uniforms.rotationMode, modeIndex);
  ...
  private getUniformLocations() {
    ...
    if (!projectionDepth || !lineWidth || !time || !rotationMode) {
      throw new Error('Failed to resolve uniform locations');
    }
    return { projectionDepth, lineWidth, time, rotationMode };
  }
}
...
vec4 applyDualQuaternionRotation(vec4 position, vec4 leftQuat, vec4 rightQuat) {
  vec4 left = normalizeQuaternion(leftQuat);
  vec4 right = normalizeQuaternion(rightQuat);
  vec4 vectorQuat = position;
  vec4 temp = quaternionMultiply(left, vectorQuat);
  vec4 rotatedQuat = quaternionMultiply(temp, quaternionConjugate(right));
  return rotatedQuat;
}

void main() {
  vec4 sequentialRotation = applySixPlaneRotation(a_position4d, spatial, hyperspatial);
  vec4 dualRotation = applyDualQuaternionRotation(a_position4d, quatLeft, quatRight);
  vec4 rotated = sequentialRotation;
  if (u_rotationMode == 1) {
    rotated = dualRotation;
  }
