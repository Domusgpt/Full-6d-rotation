...
export type RotationMode = 'sequential' | 'dual-quaternion';
...
export class HypercubeCore {
  ...
  private rotationMode: RotationMode = 'sequential';
  ...
  constructor(private readonly canvas: HTMLCanvasElement, options: HypercubeCoreOptions = {}) {
    ...
    this.rotationMode = options.rotationMode ?? this.rotationMode;
  ...
  private render(time: number, deltaTime: number) {
    ...
    if (this.rotationModeDirty) {
      gl.uniform1i(this.uniforms.rotationMode, this.rotationMode === 'dual-quaternion' ? 1 : 0);
...
vec4 applyDualQuaternion(vec4 position, vec4 left, vec4 right) {
  vec4 normLeft = quatNormalize(left);
  vec4 normRight = quatNormalize(right);
  vec4 temp = quatMultiply(normLeft, position);
  return quatMultiply(temp, quatConjugate(normRight));
}
...
void main() {
  ...
  if (u_rotationMode == 0) {
    rotated = applySixPlaneRotation(rotated, spatial, hyperspatial);
  } else {
    rotated = applyDualQuaternion(rotated, quatLeft, quatRight);
  }
  vec4 matrixProjected = rotationMatrix * rotated;
  float depth;
