...
void main() {
  ...
  float sequentialNorm = max(length(sequential), 1e-5);
  float rotatedNorm = max(length(rotated), 1e-5);
  float alignment = clamp(dot(sequential, rotated) / (sequentialNorm * rotatedNorm), -1.0, 1.0);
  float geometricMix = dot(rotated, vec4(0.12, 0.18, 0.24, 0.3));
  geometricMix += dot(sequential, rotated) * 0.01;
  float harmonic = fract(harmonicBase + geometricMix * 0.12 + isoPhase * 0.33 + alignment * 0.17);
...
void main() {
  ...
  float coherence = clamp(v_alignment, 0.0, 1.0);
  float coherenceGlow = mix(0.72, 1.24, coherence);
  float coherenceHue = (coherence - 0.5) * 0.14;

  vec3 color = (v_color + vec3(0.08 + coherenceHue, 0.15, 0.32) * v_chiral) * shimmer * (0.7 + depthFade * 0.5);
  color *= coherenceGlow;
  color += vec3(noise);
  float alpha = clamp(0.24 + energy * 0.58 + chaos * 0.17 + v_chiral * 0.2, 0.2, 1.0);
  alpha = clamp(alpha * mix(0.78, 1.16, coherence), 0.2, 1.0);
