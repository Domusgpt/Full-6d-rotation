...
export type RotationComputationMode = 'sequential' | 'matrix' | 'dualQuaternion';

const ROTATION_MODE_TO_INDEX: Record<RotationComputationMode, number> = {
  sequential: 0,
  matrix: 1,
  dualQuaternion: 2
};
...
export interface HypercubeCoreOptions {
  ...
  rotationMode?: RotationComputationMode;
}
...
export class HypercubeCore {
  ...
  private rotationMode: RotationComputationMode = 'sequential';
  private rotationModeIndex = ROTATION_MODE_TO_INDEX['sequential'];
  ...
  constructor(private readonly canvas: HTMLCanvasElement, options: HypercubeCoreOptions = {}) {
    ...
    if (options.rotationMode) {
      this.setRotationMode(options.rotationMode);
    }
    this.resize();
  ...
  setRotationMode(mode: RotationComputationMode) {
    if (this.rotationMode === mode) return;
    this.rotationMode = mode;
    this.rotationModeIndex = ROTATION_MODE_TO_INDEX[mode];
  ...
  private render(time: number, deltaTime: number) {
    ...
    if (this.rotationModeDirty) {
      gl.uniform1i(this.uniforms.rotationMode, this.rotationModeIndex);
...
vec4 applyDualQuaternionRotation(vec4 position, vec4 leftQuat, vec4 rightQuat) {
  vec4 left = quaternionNormalize(leftQuat);
  vec4 right = quaternionNormalize(rightQuat);
  vec4 vectorQuat = position;
  vec4 temp = quaternionMultiply(left, vectorQuat);
  vec4 rotated = quaternionMultiply(temp, quaternionConjugate(right));
  return rotated;
}
...
void main() {
  vec4 rotatedSequential = applySixPlaneRotation(a_position4d, spatial, hyperspatial);
  vec4 rotatedMatrix = rotationMatrix * a_position4d;
  vec4 rotatedDual = applyDualQuaternionRotation(a_position4d, quatLeft, quatRight);

  vec4 rotated = rotatedSequential;
  if (u_rotationMode == 1) {
  ...
  geometricMix += dot(rotatedMatrix, rotated) * 0.01;
