...
const MIN_VELOCITY_DELTA = 1e-3;
const MAX_PLANE_VELOCITY = Math.PI * 12;
...
function computeAngularVelocity(
  ...
  if (!Number.isFinite(deltaMs) || deltaMs <= 0) {
    clearRotationAngles(target);
    return;
  }
  const deltaSeconds = deltaMs * 0.001;
  if (!Number.isFinite(deltaSeconds) || deltaSeconds < MIN_VELOCITY_DELTA) {
    clearRotationAngles(target);
    return;
  }

  target.xy = clampVelocity((next.xy - previous.xy) / deltaSeconds);
  target.xz = clampVelocity((next.xz - previous.xz) / deltaSeconds);
  target.yz = clampVelocity((next.yz - previous.yz) / deltaSeconds);
  target.xw = clampVelocity((next.xw - previous.xw) / deltaSeconds);
  target.yw = clampVelocity((next.yw - previous.yw) / deltaSeconds);
  target.zw = clampVelocity((next.zw - previous.zw) / deltaSeconds);
}

function clearRotationAngles(target: RotationAngles) {
  target.xy = 0;
  target.xz = 0;
  target.yz = 0;
  target.xw = 0;
...
function clampVelocity(value: number): number {
  ...
  if (value > MAX_PLANE_VELOCITY) return MAX_PLANE_VELOCITY;
  if (value < -MAX_PLANE_VELOCITY) return -MAX_PLANE_VELOCITY;
  return value;
}
