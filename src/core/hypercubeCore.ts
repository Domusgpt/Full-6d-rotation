...
export interface HypercubeCoreOptions {
  ...
  rotationComputationMode?: RotationComputationMode;
}

export type RotationComputationMode = 'sequential' | 'matrix' | 'dual-quaternion';
...
export class HypercubeCore {
  ...
  private rotationMode: RotationComputationMode = 'dual-quaternion';
  private rotationModeValue = rotationModeToIndex(this.rotationMode);
  private rotationModeDirty = true;
  ...
  private uniforms!: {
    ...
    rotationMode: WebGLUniformLocation;
  };
  ...
  constructor(private readonly canvas: HTMLCanvasElement, options: HypercubeCoreOptions = {}) {
    ...
    if (options.rotationComputationMode) {
      this.setRotationComputationMode(options.rotationComputationMode);
    } else {
      this.rotationMode = 'dual-quaternion';
      this.rotationModeValue = rotationModeToIndex(this.rotationMode);
      this.rotationModeDirty = true;
    }
  ...
  setRotationComputationMode(mode: RotationComputationMode) {
    if (this.rotationMode === mode) return;
    this.rotationMode = mode;
    this.rotationModeValue = rotationModeToIndex(mode);
    this.rotationModeDirty = true;
  }
  ...
  private render(time: number, deltaTime: number) {
    ...
    if (this.rotationModeDirty) {
      gl.uniform1i(this.uniforms.rotationMode, this.rotationModeValue);
      this.rotationModeDirty = false;
    }
  ...
  private getUniformLocations() {
    ...
    const rotationMode = this.gl.getUniformLocation(this.program, 'u_rotationMode');
    if (!projectionDepth || !lineWidth || !time || !projectionMode || !projectionParams || !rotationMode) {
      throw new Error('Failed to resolve uniform locations');
    }
    return { projectionDepth, lineWidth, time, projectionMode, projectionParams, rotationMode };
  }
}

function rotationModeToIndex(mode: RotationComputationMode): number {
  switch (mode) {
    case 'sequential':
      return 0;
    case 'matrix':
...
vec4 applyDualQuaternionRotation(vec4 position, vec4 left, vec4 right) {
  vec4 leftNorm = quaternionNormalize(left);
  vec4 rightNorm = quaternionNormalize(right);
  vec4 temp = quaternionMultiply(leftNorm, position);
  vec4 rotated = quaternionMultiply(temp, quaternionConjugate(rightNorm));
  return rotated;
}
...
void main() {
  vec4 sequential = applySixPlaneRotation(a_position4d, spatial, hyperspatial);
  vec4 matrixProjected = rotationMatrix * a_position4d;
  vec4 dualProjected = applyDualQuaternionRotation(a_position4d, quatLeft, quatRight);
  vec4 rotated = sequential;
  if (u_rotationMode == 1) {
    rotated = matrixProjected;
  } else if (u_rotationMode == 2) {
    rotated = dualProjected;
  }
  float depth;
