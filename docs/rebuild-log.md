...
## Session – Unified Verification Script

### What changed
- **Package scripts** (`package.json`) – added `test:run` for Vitest’s run mode and a chained `verify` script that runs the TypeScript checker, Vitest suite, and production build sequentially.
- **Verification guide** (`docs/testable-build.md`) – collapsed the prior multi-step checklist into the new unified command while documenting the underlying steps for operators who need to run them individually.
- **Operational docs** (`docs/system-status.md`, `hypercube_core_webgl_system.md`, `docs/refine-rotor-pipeline-continuation-playbook.md`) – updated quick references and continuation guidance to point to `npm run verify` for pre-QA automation.

### Why these pieces matter
- The **verify script** ensures every automated gate (type safety, unit coverage, production bundle) runs in the correct order before manual QA, reducing the chance of skipping a step when preparing testable builds.
- **Documentation alignment** keeps onboarding consistent: operators see the same command in the testable build checklist, system status guide, and continuation playbook, eliminating conflicting instructions that previously referenced individual commands.
- Having a dedicated **Vitest run mode alias** makes it easier for CI and local scripts to invoke the fast, non-watch suite without memorising `--run` flags, improving reproducibility across environments.

### Next checkpoints
- Integrate `npm run verify` into any CI pipeline or Git hooks introduced during the staged rebuild so automated environments mirror the documented process.
- Expand the verification script with linting once coding standards stabilise for the TypeScript rebuild.
