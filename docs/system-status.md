...
### Manual QA
1. Follow the quick-start in [`docs/extrument-recipes.md`](./extrument-recipes.md) to set up a virtual MIDI target.
2. Enable the **Extruments** panel, request WebMIDI access, and connect to the configured synth.
3. Confirm live rotation updates stream MIDI CC events (verify via synth UI or console logs).
4. Switch parserator profiles to ensure confidence readings respond immediately.
