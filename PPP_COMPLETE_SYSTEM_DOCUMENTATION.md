...
#### 5.3.8 Stage 7 – Extrument Harmonic Coupling & Sonic Symbolism
- **Dual-Rotor Uniforms**: Extend the UBO contract to ship a full 4×4 rotation matrix alongside normalized left/right quaternions so shaders can blend sequential plane passes with spin(4) rotors for perfectly coupled six-plane motion.
- **Spectral Geometry Mapper**: Synthesize an `ExtrumentSnapshot` each frame by rotating vertex constellations and mapping their w-axis displacement to phi-based pitch ladders, establishing a one-to-one correspondence between geometry and harmonic content.
- **Edge Modulation Ledger**: Treat every projected edge as a modulation bus with amplitude derived from 4D chord length; expose this ledger to the audio engine so geometric tension directly modulates filters, envelopes, or FM indices.
- **Visualizer Feedback Weave**: Surface harmonic centroid, energy budget, and dominant modulations in the control panel so operators can steer sensor focus, rotation blends, or geometry swaps to sculpt both image and sound with a single gesture.
- **Dataset Harmonization**: Capture synchronized PSP frames and extrument ledgers for ML consumers, allowing ViT/CNN models to learn the symbolic relationship between geometry, projection, and emergent spectra for HAOS-guided performance.
