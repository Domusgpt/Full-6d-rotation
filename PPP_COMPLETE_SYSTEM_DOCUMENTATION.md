...
- **Explicit SO(4) Struct**: Author `RotationUniforms.ts` exporting `RotationAngles` and `RotationDualQuaternion` interfaces, with a std140 `RotationFrame` layout combining a 4×4 rotation matrix, paired quaternions, and the raw six angles (32×float32) so shaders can choose matrix, sequential, or dual paths without extra uploads.
